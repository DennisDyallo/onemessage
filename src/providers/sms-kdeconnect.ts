...
export function resolveKdeSmsSettings(cliOverrides?: Record<string, unknown>): SmsSettings | null {
  const config = loadConfig();
  const sms = config.sms;

  if ((sms?.backend ?? "beeper") !== "kdeconnect") return null;
...
function fetchSmsConversationsViaDbus(opts?: { from?: string }): MessageFull[] {
  const settings = resolveKdeSmsSettings();
  if (!settings) return [];
...
function canReadSmsViaDbus(): boolean {
  return cliExists("dbus-send") && resolveKdeSmsSettings() !== null;
}
...
export function fetchKdeSmsInbox(opts?: {
  unread?: boolean;
  fresh?: boolean;
  from?: string;
}): void {
...
export const kdeConnectSmsBackend: MessagingProvider = {
  name: "sms",
  displayName: "SMS (KDE Connect)",

  isConfigured() {
    return cliExists("kdeconnect-cli") && resolveKdeSmsSettings() !== null;
  },
  ...
  async send(recipientId, body, opts) {
    ...
    const settings = resolveKdeSmsSettings(opts?.providerFlags);
    if (!settings) {
      return {
        ok: false,
        provider: "sms",
  ...
  async inbox(opts) {
    ...
    return inboxViaDaemon({
      ...
      fallbackFetch: () => fetchKdeSmsInbox(opts),
    });
  },
  ...
  async read(messageId, opts) {
    ...
    return message && !message.account ? message : null;
  },
};
